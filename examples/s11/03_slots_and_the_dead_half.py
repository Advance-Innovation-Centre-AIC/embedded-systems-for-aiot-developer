# 03_slots_and_the_dead_half.py - ครึ่งที่ตอบทันที กับ ครึ่งที่เงียบสิบวินาที
#
# ไฟล์นี้สอน: โมดูล tesaiot มี 27 ชื่อ แต่บน Eva ใช้ได้จริงเก้าตัว
#             ส่วนอีกสิบหกตัวส่งคำสั่งข้ามไปคอร์จอ ซึ่งบนบอร์ดนี้ประกอบมาด้วย
#             ENABLE_OPTIGA = 0 ปลายทางจึงไม่มีใครรับสาย ฝั่งเราตั้งเวลารอไว้
#             10 วินาทีต่อครั้ง แล้วค่อยยอมแพ้ ไม่มีข้อความบอกว่าเพราะอะไร
#             ไฟล์นี้จับเวลาให้ดูกับตา จะได้ไม่ต้องไปค้นพบเองตอนนั่งดีบัก
# ดูที่จอ   : ครึ่งบน slots() ตอบทันที ครึ่งล่างเลขวินาทีเดินขึ้นจนครบสิบแล้วแดง
# กับดัก    : อย่าเผลอเรียกกลุ่มสิบหกตัวในลูป สามบรรทัดคือจอค้างครึ่งนาที
#
...
ui.screen()
time.sleep_ms(200)
ui.Label("เก้าตัวที่ตอบ กับ สิบหกตัวที่เงียบ", x=20, y=12, color=COL_TEXT,
         value=24)
...
lcd.clear()
lcd.console("<h2>คาบ 11 - ครึ่งที่ใช้ได้ กับ ครึ่งที่ตายแล้ว</h2>")
...
lcd.print("<b>กำลังเรียก license_verify() - จอจะนิ่งราวสิบวินาที</b>")
l_d1.color(COL_WARN)
...
lcd.print("<b>กำลังเรียก device_id() - อีกสิบวินาที</b>")
l_d2.color(COL_WARN)
...
l_verdict.text("สิบหกตัวในกลุ่มนี้เป็นแบบนี้ทุกตัวบน Eva")
ui.poll()
...
print("         จาก CM33 ตรง ๆ ไม่ต้องพึ่งคอร์จอ - ดูสไลด์โบนัสท้ายคาบ")
