...
# อ่านเก็บไว้ให้อยู่แล้วทุก 200 ms เพราะฉะนั้นเรียก sensors.pot.read() ได้เลย
#
# sensors.init() และ sensors.scan() บนบอร์ดนี้จะโยน OSError ทันที ไม่ใช่ค้าง
# ใครเผลอเรียกจะเห็น error แล้วสคริปต์หยุด บอร์ดไม่ตาย ไม่ต้องถอดไฟ
#
# ข้อควรรู้ข้อเดียวของเส้นทางนี้: หลังรีเซ็ต คอร์จอเริ่มตอบเรื่องเซนเซอร์
# ราว 13 วินาที การอ่านครั้งแรกจึงนิ่งได้ถึง 16 วินาที ครั้งถัดไปเร็วปกติ
...
#
# ทำไมเป็น 0.2 ไม่ใช่ 0.5: examples/s05/06_ema_time_constant.py แปลง alpha ให้เป็นเวลาจริง
...
# และถ้าสัญญาณของทีมมีค่าหลุดโด่ง ๆ เป็นครั้งคราว examples/s05/07_median_beats_mean.py
...
while True:
    ...
    try:
        ...
        # โวลต์บนบอร์ดนี้ไม่ได้วัดมาตรง ๆ แต่คำนวณจากเปอร์เซ็นต์คูณแรงดันอ้างอิง 3.3 V
        # และค่าอ้างอิงตัวนั้นยังไม่มีใครเอามิเตอร์ไปจิ้มขา P15[1] ยืนยัน
        # จึงใช้ดูแนวโน้มได้ แต่อย่าเพิ่งเอาไปเถียงกับมัลติมิเตอร์
        # เหตุผลที่ต้องระวังคำว่า "โวลต์" ขนาดนี้ examples/s05/05_adc_counts_to_volts.py อธิบายไว้ครบ
        ...
        # แต่วันที่เอาค่านี้ไปสั่งของจริง จะต้องรู้เพิ่มอีกเรื่อง - examples/s05/01_capsense_dimmer.py
        # แสดงว่าค่าสุดท้ายจะค้างอยู่ตอนปล่อยนิ้ว ไม่ได้กลับเป็นศูนย์ ต้องดูว่าค่าหยุดเปลี่ยนกี่รอบแล้ว
        # จึงจะสรุปว่านิ้วออก และ examples/s05/02_capsense_menu_wet_hand.py ต่อยอดเป็นเมนูที่ตอบกลับ
    ...
    if sec != last_sec:
        ...
        # ความสั่นจะถูกซ่อนไปเอง แล้วบทเรียนทั้งคาบก็หายไปด้วย
